...
1. 첫 페이지 요청 시에는 SEO와 초기 로딩 속도 향상을 위해 SSR로 완성된 HTML을 내려주고, 이후 페이지 이동 시에는 클라이언트 단에서 필요한 데이터만 가져와 렌더링함으로써 페이지 깜빡임 없는 자연스러운 사용자 경험을 주기 위함이다.

2. MPA 방식에서는 클라이언트에서 관리하는 상태(state)가 페이지 이동 시마다 초기화되는 문제가 있다. React Router의 SSR 모드에서는 클라이언트 단에서 상태 관리를 유지하면서도, 필요한 데이터만 가져와 렌더링함으로써 이러한 문제를 해결할 수 있다.

...
Prerendering은 빌드 시점에 페이지를 미리 렌더링해 정적 HTML 파일로 생성하는 방식이다. 요청 시에는 서버가 매번 렌더링하지 않고 생성된 정적 HTML을 그대로 응답하므로 초기 응답이 빠르고, 콘텐츠가 HTML에 포함되어 있어 SEO에도 유리하다. 다만 결과는 빌드 시점의 스냅샷이므로 최신 데이터가 반드시 필요하면 한계가 있으며, 필요하다면 ISR처럼 revalidate time을 지정해 정적 결과를 재생성하거나, 하이드레이션 이후 클라이언트에서 재요청/재검증(SWR, 폴링 등) 으로 일부 데이터를 최신화할 수 있다.
...
SSR은 페이지 요청이 발생할 때마다 서버에서 컴포넌트를 렌더링해 HTML을 생성하고 클라이언트에 전송하는 방식이다. 초기 응답에 콘텐츠가 포함되므로 SEO에 유리하며, 인증/개인화처럼 요청 시점에만 결정되는 데이터나 자주 변하는 데이터를 반영하기 적합하다. 다만 서버 렌더링 비용에 따라 TTFB(Time To First Byte)가 증가할 수 있어 캐싱 전략과 함께 고려하는 것이 좋다.

...
하지만 클래스를 사용한다고 해서 자바스크립트가 프로토타입 기반이라는 사실이 변하는 것은 아니다. 그래서 클래스 문법은 단지 생성자 함수와 프로토타입 체인을 더 쉽게 사용할 수 있도록 하는 문법적 설탕(syntactic sugar)에 불과하다고도 한다.
...
위처럼 클래스 선언을 한다는 것은 생성자 함수를 정의하는 것과 동일하다.
